;; 962efb355cf4c789951178b5a2cfa817d05ed8be3241b33bbbb53fbd35bf4506
      ...
      (define-constant DEX-TOKEN 'STV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RJ5XDY2.simple13-faktory)
      (define-constant PRE-CONTRACT 'STV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RJ5XDY2.simple13-pre-faktory)
      ...
      (define-public (buy (ft <faktory-token>) (ubtc uint))
        (begin
          ...
          (try! (contract-call? SBTC transfer fee tx-sender FEE-RECEIVER none)) ;; Rafa put back on (- fee pre-fee)
          ;; Rafa put back on: (try! (contract-call? SBTC transfer pre-fee tx-sender PRE-CONTRACT none))
          ;; Rafa put back on: (try! (as-contract (contract-call? PRE-CONTRACT create-fees-receipt pre-fee)))        
          (try! (contract-call? SBTC transfer stx-in tx-sender (as-contract tx-sender) none))
            ...
            (if (>= new-stx TARGET_STX)
                ...
                (try! (as-contract (contract-call? ft transfer originator-amount tx-sender ORIGINATOR none)))
                  (try! (as-contract 
                         (contract-call? 
                           'ST295MNE41DC74QYCPRS8N37YYMC06N6Q3VQDZ6G1.xyk-core-v-1-2 
                               create-pool 
                               'STV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RJ5XDY2.xyk-pool-sbtc-simple13-v-1-1
                               SBTC 
                               ft
                               amm-ustx 
                               amm-amount 
                 ...
                 ;; Rafa put back on: (try! (as-contract (contract-call? PRE-CONTRACT toggle-bonded)))
      ...
      (define-public (sell (ft <faktory-token>) (amount uint))
        (begin
          ...
          (let (
            ...
            (try! (as-contract (contract-call? SBTC transfer fee tx-sender FEE-RECEIVER none))) ;; Rafa put back on (- fee pre-fee)
            ;; Rafa put back on: (try! (contract-call? SBTC transfer pre-fee tx-sender PRE-CONTRACT none))
            ;; Rafa put back on: (try! (as-contract (contract-call? PRE-CONTRACT create-fees-receipt pre-fee)))        
            (var-set stx-balance new-stx)
            (var-set ft-balance new-ft)
      ...
      ;; boot dex
        (begin
          ...
          (var-set stx-balance u0)
          (var-set open true) ;; remove this when ready to link up to pre-contract mainnet
            (print { 
                ...
                poolName: 'STV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RJ5XDY2.xyk-pool-sbtc-simple13-v-1-1
           })
          (ok true))